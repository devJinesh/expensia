...
public class BudgetServiceImpl implements BudgetService {
    ...
    public ResponseEntity<ApiResponseDto<?>> createBudget(BudgetRequest budgetRequest) throws UserNotFoundException, UserServiceLogicException {
        ...
        try {
            Budget budget = budgetRepository.findByUserIdAndMonthAndYear(budgetRequest.getUserId(), budgetRequest.getMonth(), budgetRequest.getYear());
            if (budget == null){
                budget = new Budget(
                        budgetRequest.getUserId(), budgetRequest.getAmount(), budgetRequest.getMonth(), budgetRequest.getYear()
                );
            }
            else {
