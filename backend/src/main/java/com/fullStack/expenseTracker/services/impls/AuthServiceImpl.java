...
public class AuthServiceImpl implements AuthService {
    ...
    public ResponseEntity<ApiResponseDto<?>> verifyRegistrationVerification(String code) throws UserVerificationFailedException {
        ...
        user.setResendCount(0); // Reset resend count on successful verification
        userRepository.save(user);
    ...
    public ResponseEntity<ApiResponseDto<?>> resendVerificationCode(String email) throws UserNotFoundException, UserServiceLogicException {
        ...
        if (user.getResendCount() >= 5) {
            return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body(new ApiResponseDto<>(
                    ApiResponseStatus.FAILED, HttpStatus.TOO_MANY_REQUESTS, 
                    "Maximum resend attempts exceeded. Please try again after 24 hours."
            ));
        }

        if (user.getLastResendTime() != null) {
            long timeSinceLastResend = System.currentTimeMillis() - user.getLastResendTime().getTime();
            long twoMinutesInMs = 2 * 60 * 1000;
            
            if (timeSinceLastResend < twoMinutesInMs) {
                long remainingSeconds = (twoMinutesInMs - timeSinceLastResend) / 1000;
                return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body(new ApiResponseDto<>(
                        ApiResponseStatus.FAILED, HttpStatus.TOO_MANY_REQUESTS, 
        ...
        try {
            ...
            user.setLastResendTime(new Date());
